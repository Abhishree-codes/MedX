...
const handleUserApptPost=(postData,selectedDate,timeslot,userData)=>{
    axios.post("https://bookish-spoon-json-server.bhishree18.repl.co/appointments",{
        "name":userData.name,
...
const getDoctor=(id,doctorid)=>{
    axios.get(`https://bookish-spoon-json-server.bhishree18.repl.co/services/${id}`).then((res)=>{
...
const putRequest=(values)=>{
  ...
  axios.put(`https://bookish-spoon-json-server.bhishree18.repl.co/services/1`,{
    ...postDataAfterChange
  }).then((res)=>{
    console.log(res.data)
  }).catch((error)=>{
...
//slots here 
   ...
   <Box  p={4}  maxW={"11xl"}>
      <Stack spacing={4} as={Container} maxW={'11xl'} textAlign={'center'} mb={10}>
       ...
       //error is caused by md
        ...
        width={"50%"}
          ...
          color={"black"} fontSize='2xl' fontWeight={"medium"}
            ...
            px={3}>
            Hourly Rate: ₹ {SingleDoctor.price}
          </Text>
          <Text
            textAlign={'center'}
